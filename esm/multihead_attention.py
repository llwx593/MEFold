...
class MultiheadAttention(nn.Module):
    ...
    def _linear_q(self, x):
        return self.q_proj(x)
     
    @torch.jit.ignore
    def _q_linear_chunk(self,
        x: torch.Tensor,
        chunk_size: int,
    ) -> torch.Tensor:
        print("!!!qlinear_chunk",chunk_size)
        # print(x.shape)
        return chunk_layer(
            partial(
                self._linear_q, 
    ...
    def _linear_k(self, x):
        return self.k_proj(x)
     
    @torch.jit.ignore
    def _k_linear_chunk(self,
        x: torch.Tensor,
        chunk_size: int,
    ) -> torch.Tensor:
        print("!!!klinear_chunk",chunk_size)
        return chunk_layer(
            partial(
                self._linear_k, 
            ),
    ...
    def _linear_v(self, x):
        return self.v_proj(x)
     
    @torch.jit.ignore
    def _v_linear_chunk(self,
        x: torch.Tensor,
        chunk_size: int,
    ) -> torch.Tensor:
        print("!!!vlinear_chunk",chunk_size)
        return chunk_layer(
            partial(
                self._linear_v, 
            ),
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            ...
            assert key_padding_mask.size(1) == src_len(q, k.transpose(1, 2))
