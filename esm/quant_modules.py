...
class Quant_Linear(Module):
    ...
    def __init__(self, weight_bit, full_precision_flag=False):
        ...
        self.dequantize_function = AsymmetricQuantFunction_de.apply
    ...
    def forward(self, x):
        ...
        x_transform = x.squeeze().data.detach()
        # print(x_transform.shape)
        w_min = w_transform.min(dim=1).values
        w_max = w_transform.max(dim=1).values      
        # x_min = x_transform.min(dim=0).values
        # x_max = x_transform.max(dim=0).values
        # print(x_min.shape)
        if not self.full_precision_flag:
