...
from openfold.model.primitives import Linear, LayerNorm, _chunk_matmul
...
class TriangleMultiplicativeUpdate(nn.Module):
    ...
    def _combine_projections(self,
        ...
        if(_inplace_chunk_size is not None):
            ...
            p = a
            # p = _chunk_matmul(a, b, _inplace_chunk_size)
        else:
            # print("_combine_projections")
            # print(a.shape)
            # print(b.shape)
            p = torch.matmul(a, b)
            # p = _chunk_matmul(a, b, 4)

        return permute_final_dims(p, (1, 2, 0))
    
    def quantize(self, num_bits=8):
        self.linear_a_g = QLinear(self.linear_a_g, qi=True, qo=True, num_bits=num_bits)
        self.linear_a_p = QLinear(self.linear_a_p, qi=False, qo=True, num_bits=num_bits)
        self.linear_b_g = QLinear(self.linear_b_g, qi=False, qo=True, num_bits=num_bits)
        self.linear_b_p = QLinear(self.linear_b_p, qi=False, qo=True, num_bits=num_bits)
        # self.init_time+=self.linear_b_p.time
        self.linear_z = QLinear(self.linear_z,qi=True, qo=True, num_bits=num_bits)
        self.linear_g = QLinear(self.linear_g,qi=False, qo=True, num_bits=num_bits)
    
    def freeze(self):
        self.linear_a_g.freeze()
        self.linear_a_p.freeze(self.linear_a_g.qo)
        self.linear_b_g.freeze(self.linear_a_p.qo)
        self.linear_b_p.freeze(self.linear_b_g.qo)
        self.linear_z.freeze()
        self.linear_g.freeze(self.linear_z.qo)
    ...
    def quantize_forward(self, 
        ...
        if(is_fp16_enabled()): 
            with torch.cuda.amp.autocast(enabled=False):
                x = self._combine_projections(a.float(), b.float())    #matmul
        else:
            # print("matmul-tmupdate")
    ...
    def quantize_inference(self, 
        ...
        z = self.layer_norm_in(z)
        a = mask
        a = a * self.sigmoid(self.linear_a_g.quantize_inference(z)) 
        a = a * self.linear_a_p.quantize_inference(z)
        b = mask
        b = b * self.sigmoid(self.linear_b_g.quantize_inference(z))
        b = b * self.linear_b_p.quantize_inference(z)
        ...
        del a, b
        x = self.layer_norm_out(x)
        x = self.linear_z.quantize_inference(x)
        g = self.sigmoid(self.linear_g.quantize_inference(z))
        x = x * g
    ...
    def forward(self, 
        ...
        """
        # if(inplace_safe and _inplace_chunk_size):
        #     # print("inference forward!!!!!")
        #     x = self._inference_forward(     #matmul
        #         z, 
        #         mask, 
        ...
        if(is_fp16_enabled()): 
            with torch.cuda.amp.autocast(enabled=False):
                x = self._combine_projections(a.float(), b.float(),_inplace_chunk_size=_inplace_chunk_size)    #matmul
        else:
            # print("matmul-tmupdate")
