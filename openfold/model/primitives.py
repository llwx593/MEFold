...
def trunc_normal_init_(weights, scale=1.0, fan="fan_in"):
    ...
    size = _prod(shape)
    weights = truncnorm.rvs(a=a, b=b, loc=0, scale=std, size=size)
    weights = np.reshape(weights, shape)
    with torch.no_grad():
        # weights.copy_(torch.tensor(samples, device=weights.device))
        # weights = torch.tensor(samples).clone()
        weights = torch.from_numpy(weights)
        
    # return time.time()-s1
    # print("trunc_normal_init_time:",time.time()-s1)
...
class Linear(nn.Linear):
    ...
    def __init__(
        ...
        #     else:
        #         if init == "default":   
        #             t = lecun_normal_init_(self.weight)
        #             self.time=t
        #         elif init == "relu":  
        #             t = he_normal_init_(self.weight)
        #             self.time=t
        #         elif init == "glorot":
...
def quantize_to_int8(tensor, clip_max, quant_range = 127):
    ...
    outputs = torch.clamp((tensor.type(torch.float32) * scale).round(), min_bound, max_bound)

    quant_tensor = outputs.type(torch.int8)
    
    return quant_tensor

def _matmul(query, key):
...
def _chunk_matmul(
    ...
    return chunk_layer(
        ...
        no_batch_dims=len(query.shape[:-2]), 
        )
...
def _attention(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, biases: List[torch.Tensor]) -> torch.Tensor:
    ...
    # print("matmul chunk here!!!!")
    # a = _chunk_matmul(query=query, key=key, chunk_size=1)
    
    # print(query.shape)
    # q_amax = query.abs().max()
    # k_amax = key.abs().max()
    # i8_q = quantize_to_int8(query, q_amax)
    # i8_k = quantize_to_int8(key, k_amax)
    # i32_a = torch.matmul(i8_q, i8_k)
    # a = (i32_a * q_amax/127 * k_amax/127)

    for b in biases:
        a += b

    a = softmax_no_cast(a, -1)
    # print("!!!chunk andd_softmax here")
    # a = _chunk_add_softmax(a, biases, 8)

    # [*, H, Q, C_hidden]
    a = torch.matmul(a, value)
    # a = _chunk_matmul(query=a, key=value, chunk_size=1)
...
class Attention(nn.Module):
    ...
    def quantize(self, num_bits=8):
        self.linear_q = QLinear(self.linear_q, qi=True, qo=True, num_bits=num_bits)
        self.linear_k = QLinear(self.linear_k, qi=False, qo=True, num_bits=num_bits)
        self.linear_v = QLinear(self.linear_v, qi=False, qo=True, num_bits=num_bits)
        
        if self.gating:
            # print("!!!linear_g")
            self.linear_g = QLinear(self.linear_g, qi=True, qo=True, num_bits=num_bits)
        self.linear_o = QLinear(self.linear_o, qi=True, qo=True, num_bits=num_bits)

    def freeze(self):
        self.linear_q.freeze()
        self.linear_k.freeze(self.linear_q.qo)
        self.linear_v.freeze(self.linear_q.qo)
        self.linear_g.freeze()
        self.linear_o.freeze()
    ...
    def _prep_qkv_quantize_forward(self,
        q_x: torch.Tensor, 
        kv_x: torch.Tensor
    ) -> Tuple[
        torch.Tensor, torch.Tensor, torch.Tensor
    ...
    def _prep_qkv_quantize_inference(self,
        q_x: torch.Tensor, 
        kv_x: torch.Tensor
    ) -> Tuple[
        torch.Tensor, torch.Tensor, torch.Tensor
        ...
        # [*, Q/K/V, H * C_hidden]
        q = self.linear_q.quantize_inference(q_x)
        k = self.linear_k.quantize_inference(kv_x)
        v = self.linear_v.quantize_inference(kv_x)
    ...
    def _wrap_up_quantize_forward(self,
        o: torch.Tensor, 
        q_x: torch.Tensor
    ) -> torch.Tensor:
        if(self.linear_g is not None):
    ...
    def _wrap_up_quantize_inference(self,
        o: torch.Tensor, 
        q_x: torch.Tensor
    ) -> torch.Tensor:
        if(self.linear_g is not None):
            g = self.sigmoid(self.linear_g.quantize_inference(q_x))
        ...
        # [*, Q, C_q]
        o = self.linear_o.quantize_inference(o)
    ...
    def quantize_forward(
        ...
        # [*, H, Q/K, C_hidden]
        # q, k, v = self._prep_qkv(q_x, kv_x)
        q, k, v = self._prep_qkv_quantize_forward(q_x, kv_x)
        ...
        # o = self._wrap_up(o, q_x)
        o = self._wrap_up_quantize_forward(o, q_x)
    ...
    def quantize_inference(
        ...
        # [*, H, Q/K, C_hidden]
        # q, k, v = self._prep_qkv(q_x, kv_x)
        q, k, v = self._prep_qkv_quantize_inference(q_x, kv_x)
        ...
        # o = self._wrap_up(o, q_x)
        o = self._wrap_up_quantize_inference(o, q_x)
