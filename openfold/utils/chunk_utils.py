...
class MemoryIntimeLogger:
    def __init__(self, device_id: int):
        ...
        logging.info(f'chunk memoryRecord Initializing: Used memory = {gpu.memoryUsed}MiB, Free memory = {gpu.memoryFree}MiB, '
...
def chunk_layer(
    ...
    """
    # chunk_memory = MemoryIntimeLogger(0)
    if not (len(inputs) > 0):
    ...
    # chunk_memory.log("prepped inputs")
    # print("no_batch_dim",no_batch_dims)
    # print("ori_batch_dim",orig_batch_dims)
    flat_batch_dim = 1
    for d in orig_batch_dims:
        flat_batch_dim *= d

    # print("flat_batch_dim",flat_batch_dim)
    
    no_chunks = flat_batch_dim // chunk_size + (
        flat_batch_dim % chunk_size != 0
    )
    # print("no_chunks",no_chunks)
    i = 0
    out = prepped_outputs
    for _ in range(no_chunks):
        ...
        # chunk_memory.log("complete computing")
